...
// Pure JS Base64 to uppercase Hex converter
function base64ToHex(b64) {
    ...
    for (var i = 0; i < clean.length; i++) {
        var val = b64chars.indexOf(clean.charAt(i));
        if (val === -1) continue;
        buf = (buf << 6) | val;
        bits += 6;
        if (bits >= 8) {
            bits -= 8;
            var byte = (buf >>> bits) & 0xff;
...
// AES-128-ECB Encryptor using embedded CryptoJS
function encryptSg(pid, timestamp) {
    var text = pid + '-' + timestamp;
    var keyStr = CryptoJS.MD5(text).toString().substring(0, 16);
...
function parseCardList(html) {
    var list = [];
    if (!html) return list;
    var cardRegex = /<div class="movie-card">([\s\S]*?)<\/div>\s*<\/div>/g;
    var match;
    while ((match = cardRegex.exec(html)) !== null) {
        var card = match[1];
        var hrefMatch = card.match(/href=["'](\/[^"']+\.htm)["']/);
        var titleMatch = card.match(/<h4>([^<]+)<\/h4>/) || card.match(/title=["']([^"']+)["']/);
        var picMatch = card.match(/data-src=["']([^"']+)["']/) || card.match(/src=["']([^"']+)["']/);
        var remarkMatch = card.match(/<div class="episode-badge">([^<]+)<\/div>/);

        if (hrefMatch && titleMatch) {
            var picUrl = picMatch ? picMatch[1] : '';
            if (picUrl && picUrl.indexOf('http') !== 0) {
                picUrl = siteHost + picUrl;
            }
            list.push({
                vod_id: hrefMatch[1],
 ...
 */
function homeVod() {
    var html = httpReq(siteHost + '/s/all', {
...
function category(tid, pg, filter, extend) {
    ...
    if (tid.indexOf('?') !== -1) {
        var parts = tid.split('?');
        url = siteHost + '/s/' + parts[0] + '/' + pg + '?' + parts[1];
    } else {
        url = siteHost + '/s/' + tid + '/' + pg;
    }
...
function detail(id) {
    ...
    var title = (html.match(/<h1[^>]*class=["']movie-title["'][^>]*>([\s\S]*?)<\/h1>/i) || ['',''])[1].replace(/<[^>]+>/g, '').trim();
    if (!title) {
        title = (html.match(/<title>([^<]+)<\/title>/i) || ['',''])[1].split('-')[0].trim();
    }

    var picMatch = html.match(/<div class="movie-poster">[\s\S]*?src=["']([^"']+)["']/i);
    var pic = picMatch ? picMatch[1] : '';
    if (pic && pic.indexOf('http') !== 0) {
        pic = siteHost + pic;
    }

    var descMatch = html.match(/<div class="desc">([\s\S]*?)<\/div>/i);
    var desc = descMatch ? descMatch[1].replace(/<br\s*\/?>/gi, '\n').replace(/<[^>]+>/g, '').trim() : '';

    var yearMatch = html.match(/(\d{4})-\d{2}-\d{2}\s*\(.*?\)\s*上映/);
    var year = yearMatch ? yearMatch[1] : '';

    var directors = [];
    var dMatches = html.matchAll(/href=["']\/director\/([^"']+)["']/g);
    for (var d of dMatches) {
        directors.push(decodeURIComponent(d[1]));
    }

    var actors = [];
    var aMatches = html.matchAll(/href=["']\/performer\/([^"']+)["']/g);
    for (var a of aMatches) {
        actors.push(decodeURIComponent(a[1]));
    }

    var epRegex = /<a class="play-item"[^>]*href=["']([^"']+)["'][^>]*>([^<]+)<\/a>/g;
    var episodes = [];
    var epMatch;
    while ((epMatch = epRegex.exec(html)) !== null) {
        episodes.push(epMatch[2].trim() + '$' + epMatch[1]);
    }

    if (episodes.length === 0) {
        var singlePlay = html.match(/href=["'](\/play\/\d+-\d+\.htm)["']/);
        if (singlePlay) {
            episodes.push('正片$' + singlePlay[1]);
        }
    }
    ...
    var vod = {
        ...
        vod_area: '',
        vod_remarks: '',
        vod_actor: actors.slice(0, 10).join(', '),
...
function play(flag, id, flags) {
    var playUrl = (id.indexOf('http') === 0) ? id : (siteHost + (id.indexOf('/') === 0 ? id : ('/' + id)));
    ...
    var pidMatch = html.match(/var\s+pid\s*=\s*(\d+);/);
    if (!pidMatch) {
        return JSON.stringify({ parse: 0, url: '', header: {} });
    }
    ...
    var sg = encryptSg(pid, timestamp);

    var linesApi = siteHost + '/lines?t=' + timestamp + '&sg=' + sg + '&pid=' + pid;
    var linesResp = httpReq(linesApi, {
        headers: {
    ...
    if (linesResp) {
        try {
            var data = (typeof linesResp === 'string') ? JSON.parse(linesResp) : linesResp;
            if (data && data.code === 0 && data.data) {
                // Priority 1: url3 direct MP4 / CDN video
                if (data.data.url3) {
                    var uList = data.data.url3.split(',');
                ...
                // Priority 2: direct m3u8 if url3 is empty
...
function search(wd, quick, pg) {
    pg = parseInt(pg, 10) || 1;
    var searchUrl = siteHost + '/search/' + encodeURIComponent(wd || '');
    var html = httpReq(searchUrl, {
        headers: { 'User-Agent': DEFAULT_UA, 'Referer': siteHost + '/' }
    });

    var list = [];
    if (html && html.indexOf('verifyCode') === -1) {
